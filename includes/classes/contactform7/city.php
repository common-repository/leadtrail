...
/**
 ** A base module for [leadcity] and [leadcity*]
 **/

/* form_tag handler */

add_action('wpcf7_init', 'nbcpf_add_form_tag_leadcity');

function nbcpf_add_form_tag_leadcity()
{
  wpcf7_add_form_tag(
    array('leadcity', 'leadcity*'),
    'nbcpf_leadcity_form_tag_handler',
    array('name-attr' => true)
  );
}

function nbcpf_leadcity_form_tag_handler($tag)
{
  if (empty($tag->name)) {
    return '';
  }
...
add_filter('wpcf7_validate_leadcity', 'nbcpf_leadcity_validation_filter', 10, 2);
add_filter('wpcf7_validate_leadcity*', 'nbcpf_leadcity_validation_filter', 10, 2);

function nbcpf_leadcity_validation_filter($result, $tag)
{
  $type = $tag->type;
  $name = $tag->name;
...
add_action('wpcf7_admin_init', 'nbcpf_add_tag_generator_leadcity', 20);

function nbcpf_add_tag_generator_leadcity()
{
  $tag_generator = WPCF7_TagGenerator::get_instance();
  $tag_generator->add(
    'leadcity',
    __('lead city', 'leadtrail'),
    'nbcpf_tag_generator_leadcity'
  );
}

function nbcpf_tag_generator_leadcity($contact_form, $args = '')
{
