...
/**
 ** A base module for [leadzipcode] and [leadzipcode*]
 **/

/* form_tag handler */

add_action('wpcf7_init', 'nbcpf_add_form_tag_leadzipcode');

function nbcpf_add_form_tag_leadzipcode()
{
  wpcf7_add_form_tag(
    array('leadzipcode', 'leadzipcode*'),
    'nbcpf_leadzipcode_form_tag_handler',
    array('name-attr' => true)
  );
}

function nbcpf_leadzipcode_form_tag_handler($tag)
{
  if (empty($tag->name)) {
    return '';
  }
...
add_filter('wpcf7_validate_leadzipcode', 'nbcpf_leadzipcode_validation_filter', 10, 2);
add_filter('wpcf7_validate_leadzipcode*', 'nbcpf_leadzipcode_validation_filter', 10, 2);

function nbcpf_leadzipcode_validation_filter($result, $tag)
{
  $type = $tag->type;
  $name = $tag->name;
...
add_action('wpcf7_admin_init', 'nbcpf_add_tag_generator_leadzipcode', 20);

function nbcpf_add_tag_generator_leadzipcode()
{
  $tag_generator = WPCF7_TagGenerator::get_instance();
  $tag_generator->add(
    'leadzipcode',
    __('lead zipcode', 'leadtrail'),
    'nbcpf_tag_generator_leadzipcode'
  );
}

function nbcpf_tag_generator_leadzipcode($contact_form, $args = '')
{
